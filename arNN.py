...
from pytorch_models import arNN
import time
...
for dataset in datasets:
    ...
    for layers in model_params['n_layers']:
        for hidden in model_params['n_hiddens']:
            ...
            model = arNN(seq_len = seq_len,
                            l_alphabet = alphabet_len,
                            n_layers = layers,
                            h_dim = hidden).to(device)
            
            optimizer = torch.optim.Adam(model.parameters(), lr=learn_rate, weight_decay=model_params['l2_reg_hoca'])
                    ...
                    if (val_loss < best_loss).any():
                        ...
                        for i in range(seq_len):
                            if val_loss[i] < best_loss[i]:
                                ...
                                if i > 0:
                                    best_model.FFNets[i-1] = copy.deepcopy(model.FFNets[i-1])
                                else:
