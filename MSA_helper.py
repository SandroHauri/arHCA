# Originally from Debora Marks, Deepsequence. Adapted to fit with pytorch implementation. Removed Theano dependence to calculate sequence weights.
...
class DataHelper:
    ...
    def gen_full_alignment(self):
        ...
        if self.calc_weights:
            def weightfun(X, cutoff, chunk_size=1000):
                """
                Compute weights in chunks to avoid huge memory usage.
                
                Parameters
                ----------
                X : np.ndarray
                    Input array of shape (n_samples, h, w).
                cutoff : float
                    Threshold scalar.
                chunk_size : int
                ...
                -------
                weights : np.ndarray
                    Array of shape (n_samples,) with computed weights.
                """
                X_flat = X.reshape(X.shape[0], -1)  # (n_samples, n_features)
                n_samples = X_flat.shape[0]
                weights = np.zeros(n_samples, dtype=float)

                # Precompute denominators for each row
                denom = np.sum(X_flat * X_flat, axis=1)  # (n_samples,)

                for start in tqdm(range(0, n_samples, chunk_size)):
                    end = min(start + chunk_size, n_samples)
                    chunk = X_flat[start:end]  # (chunk_size, n_features)

                    # Compute similarities for this chunk against all rows
                    sims = (chunk @ X_flat.T) / denom[None, :]  # (chunk_size, n_samples)

                    # Count threshold exceedances
                    counts = np.sum(sims > 1 - cutoff, axis=1)

                    # Reciprocal
                    weights[start:end] = np.where(counts > 0, 1.0 / counts, 0.0)

                return weights
            # Usage:
            self.weights = weightfun(self.x_train, self.theta)
